	...
	export default{
		...
		methods: {
			...
			onDelete(){
				...
				axios.delete('http://localhost/LAV/public/api/quote/'+ this.qt.id+'?token='+token)
				.then(
			...
			onUpdate() {
				...
				axios.put('http://localhost/LAV/public/api/quote/' +this.qt.id+'?token='+token, {content: this.editValue})
				.then(
						response => console.log(response)
					)
				.catch(
